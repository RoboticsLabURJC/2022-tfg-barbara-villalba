...
class QLearning:
    def __init__(self):
        ...
        #self.QTable = np.genfromtxt('/home/bb6/pepe_ws/src/qlearning/trainings/14-abril/q_table.csv', delimiter=',',skip_header=1,usecols=range(1,22))
    ...
    def functionQLearning(self,state,next_state,action,reward):
        ...
        self.QTable[state, action] = round(self.QTable[state, action] + self.alpha * (reward + self.gamma * np.max(self.QTable[next_state]) - self.QTable[state, action]),3)
    ...
    def algorithm(self,perception):
        ...
        self.client_airsim.simPause(False)
        file = "/home/bb6/pepe_ws/src/qlearning/trainings/airsim/15-abril/fotos/foto-episodio" + str(counter_photo) + ".jpg"
...
def save_files(n_episode, epsilon, steps, acummulateReward):
    ...
    with open('/home/bb6/pepe_ws/src/qlearning/trainings/airsim/15-abril/episodes-iterations.csv', mode="a", newline="") as file_steps:
        writer_steps = csv.writer(file_steps)
        writer_steps.writerow([n_episode, steps])

    with open('/home/bb6/pepe_ws/src/qlearning/trainings/airsim/15-abril/episodes-epsilon.csv', mode="a", newline="") as file_epsilon:
        writer_epsilon = csv.writer(file_epsilon)
        writer_epsilon.writerow([n_episode,epsilon])

    with open('/home/bb6/pepe_ws/src/qlearning/trainings/airsim/15-abril/episodes-accumulated-reward.csv', mode="a", newline="") as file_reward:
    ...
    df.to_csv('/home/bb6/pepe_ws/src/qlearning/trainings/airsim/15-abril/q_table.csv')
    ...
    while (not rospy.is_shutdown()):
        try:
            if(state == 0):
                ...
                qlearning.client_airsim.moveToZAsync(-2.4, 1).join()
                time.sleep(2)
                state = 2
