...
angular_speeds = np.linspace(-0.35, 0.35, 21)
#speeds_actions = np.linspace(1.45,1.9,11, dtype=float)
#angular_speeds = np.flip(np.linspace(0.015, 0.15, 10))
...
class QLearning:
    def __init__(self):
        ...
        self.QTable = np.genfromtxt('/home/bb6/pepe_ws/src/qlearning/trainings/25-marzo/q_table.csv', delimiter=',',skip_header=1,usecols=range(1,22))
        self.accumulatedReward = 0
       

        self.MAX_EPISODES = rospy.get_param('~max_episodes')
        self.epsilon_initial = 0.99
    ...
    def reward_function(self,cx,angle,cx_prev):
        ...
        if (self.is_exit_lane(cx,error_angle_orientation,error_lane_center,cx_prev)):
            ...
            #if(error_normalised > 1.00):
                #error_normalised = 1.0
            #print("Error normalizado:" + str(error_normalised))
            
            #reward = 1- error_normalised
    ...
    def is_exit_lane(self,cx,angle_error,error_lane_center,cx_prev):
        ...
        if(is_first):
            print("Fuera del carril: " + str(status))
        return status
    ...
    def algorithm(self,perception):
        ...
        exit_lane = self.is_exit_lane(cx,error_angle,error,cx_prev)
        ...
        while(not exit_lane and (not self.is_finish_route())):
            ...
            print("Diferencia de centroides en valor absoluto: " + str(abs(cx - cx_prev)))
            ...
            #t4 = time.time()
            is_first = False    
            reward = qlearning.reward_function(cx,angle_orientation,cx_prev)
...
if __name__ == '__main__':
    ...
    with open('/home/bb6/pepe_ws/src/qlearning/trainings/25-marzo/episodes-iterations.csv', 'a') as file:
        wtr = csv.writer(file, delimiter= ' ')
        wtr.writerows(list_ep_it)

    with open('/home/bb6/pepe_ws/src/qlearning/trainings/25-marzo/episodes-epsilon.csv', 'a') as file:
        wtr = csv.writer(file, delimiter= ' ')
        wtr.writerows(list_ep_epsilon)

    with open('/home/bb6/pepe_ws/src/qlearning/trainings/25-marzo/episodes-accumulated-reward.csv', 'a') as file:
        wtr = csv.writer(file, delimiter= ' ')
        wtr.writerows(list_ep_accumulate_reward)

    df = pd.DataFrame(qlearning.QTable)
    df.to_csv('/home/bb6/pepe_ws/src/qlearning/trainings/25-marzo/q_table.csv')
