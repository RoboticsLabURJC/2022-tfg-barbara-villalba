...
ACTIONS = []
speeds_actions = np.linspace(0.5,2.0,11, dtype=float)
angular_speeds = np.linspace(-0.15, 0.15, 21)
...
class QLearning:
    def __init__(self):
        ...
        self.QTable = np.genfromtxt('/home/bb6/pepe_ws/src/qlearning/trainings/8-abril/q_table.csv', delimiter=',',skip_header=1,usecols=range(1,22))
    ...
    def algorithm(self,perception):
        ...
        self.client_airsim.simPause(False)
        file = "/home/bb6/pepe_ws/src/qlearning/trainings/8-abril/fotos/foto-episodio" + str(counter_photo) + ".jpg"
        cv2.imwrite(file ,image)
        print("FPS train: " + str(1/(t3 - t0)))
            #print("Centroide: " + str(cx))
                ...
                if (points_line_right is None or points_line_left is None):
                    ...
                    #--Detect left turn
                        ...
                        if(is_first_time):
                            ...
                            if(len(points_beetween_lines) > 35100):
...
def save_files(n_episode, epsilon, steps, acummulateReward):
    ...
    with open('/home/bb6/pepe_ws/src/qlearning/trainings/8-abril/episodes-iterations.csv', mode="a", newline="") as file_steps:
        writer_steps = csv.writer(file_steps)
        writer_steps.writerow([n_episode, steps])

    with open('/home/bb6/pepe_ws/src/qlearning/trainings/8-abril/episodes-epsilon.csv', mode="a", newline="") as file_epsilon:
        writer_epsilon = csv.writer(file_epsilon)
        writer_epsilon.writerow([n_episode, epsilon])

    with open('/home/bb6/pepe_ws/src/qlearning/trainings/8-abril/episodes-accumulated-reward.csv', mode="a", newline="") as file_reward:
        writer_reward = csv.writer(file_reward)
        writer_reward.writerow([n_episode, acummulateReward])

    df = pd.DataFrame(qlearning.QTable)
    df.to_csv('/home/bb6/pepe_ws/src/qlearning/trainings/8-abril/q_table.csv')
